...
class HermioneLocalChatV4_2:
    def __init__(self, db_path="hermione_dialog_v4_2.db"):
    ...
    def _init_db(self):
        ...
        if cursor.fetchone()[0] == 0:
            teachings = [
                ...
                # Conhecimento Técnico (Extraído do PDF)
    ...
    def _format_vtuber(self, text):
        ...
        sufixos = ["né?", "hihi!", "😏", "🥰", "😤", "✨", "🔥"]
        if random.random() > 0.7: text = f"{random.choice(prefixos)} {text}"
        if random.random() > 0.7: text = f"{text} {random.choice(sufixos)}"
        return text
...
if __name__ == "__main__":
    chat = HermioneLocalChatV4_2()
    conversa = [
        "Oi Hermione!",
        "Como você está?",
        "Tô bem, vamos falar de Python?",
        "Quais os tipos de dados?",
        "E as listas?",
        "Valeu, tchau!"
    ]
    print("--- TESTE DE CONVERSA TÉCNICA HERMIONE V4.2 ---")
    for msg in conversa:
        start = time.time()
        ...
        print(f"[Contexto: {chat.context['current_topic']}]")
        print("-" * 30)
